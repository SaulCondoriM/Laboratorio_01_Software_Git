...
public:
    ...
    //Ahora necesito que implemente una funcion dentro de mi arbol B+ que me imprima todo el recorrido que tiene los nodos, hasta llegar a un determinado registro, mi codigo de arbol B+ te lo pase al inicio.
    ...
    void searchPath(Node<T>* cursor, T key) {
    ...
    for (int i = 0; i < cursor->size + 1; ++i) {
        ...
        if (cursor->item[i] > key) {
            searchPath(cursor->children[i], key);
            return;
        }
    }

    
    searchPath(cursor->children[cursor->size], key);
}

};

void insertRecord(string filename, int index, string text) {
    ...
    if (file.is_open()) {
        while (getline(file, line)) {
            ...
            ss >> current_index;
            string current_text;
            getline(ss, current_text);
            data[current_index] = current_text;
        }
        file.close();
    } else {
    ...
    if (data.find(index) != data.end()) {
        cout << "El índice ya existe. Por favor, introduce otro índice." << endl;
        return;
    }
...
void deleteRecord(std::string filename, int index) {
    ...
    if (data.find(index) == data.end()) {
        std::cout << "El índice no existe. Por favor, introduce un índice válido." << std::endl;
        return;
    }
    ...
    if (outfile.is_open()) {
        for (auto it = data.begin(); it != data.end(); ++it) {
            if (it->first == index) {
                outfile << "\n"; // Escribir una línea en blanco en lugar del registro eliminado
            } else {
                outfile << it->first << " " << it->second << "\n";
            }
        }
...
void printRecord(BPlusTree<int>& btree, std::string filename, int index) {
    // Verifica si el índice existe en el árbol B+
    if (!btree.search(index)) {
        std::cout << "El índice no existe en el árbol B+." << std::endl;
        return;
    }

    // Si el índice existe en el árbol B+, busca el registro en el archivo
    std::ifstream file(filename);
    std::string line;
    ...
    if (file.is_open()) {
        while (getline(file, line)) {
            ...
            if (current_index == index) {
                getline(ss, current_text);
                std::cout << "Registro encontrado: " << index << " " <<current_text << std::endl;
                return;
            }
        }
        file.close();
    ...
    std::cout << "El índice existe en el árbol B+, pero no se encontró el registro correspondiente en el archivo." << std::endl;
}
...
int main() {
    ...
    string filename = "file_titanic";
    ifstream file(filename);
    string line;
    BPlusTree<int> btree(10);
    ...
    if (file.is_open()) {
        while (getline(file, line)) {
            ...
            try {
                ...
                btree.insert(stoi(index));
            } catch (invalid_argument&) {
                cerr << "Index no encontrado: " << index << '\n';
    ...
    cout<<"\n--------------IMPRESION ARBOL--------------"<<endl;
    btree.bpt_print();

    // cout<<"\n--------------RECORRIDO--------------"<<endl;

    // btree.searchPath(btree.getroot(), 891);

    btree.insert(8000);

    if(btree.search(8000)){
        cout<<"\nSI";
    }
    else{
        cout<<"\nNO";
